...
BEGIN
  ...
  IF v_type = 'term' THEN
    v_term := p_node->>'term';
    IF v_term IS NULL OR length(v_term) = 0 OR length(v_term) > 200 THEN
      RETURN false;
    END IF;
...
CREATE OR REPLACE FUNCTION public.search_job_ids_v1(
  ...
  p_min_repost_count integer DEFAULT NULL,
  p_min_seen_count integer DEFAULT NULL,
  p_posted_after timestamptz DEFAULT NULL,
  p_sort_by text DEFAULT 'posted_at',
...
AS $$
  WITH matched AS (
    ...
    WHERE public.job_search_eval(j, p_search_ast)
      ...
      AND (
        (p_kind = 'applied' AND (CASE WHEN p_application_status IS NOT NULL THEN j.status = p_application_status ELSE j.status = ANY (ARRAY['applied','interviewing','offer']) END))
        OR (p_kind NOT IN ('all','applied') AND j.status = 'new')
        OR p_kind = 'all'
      )
      AND (p_min_score IS NULL OR j.resume_score >= p_min_score)
      AND (p_max_score IS NULL OR j.resume_score <= p_max_score)
      ...
      AND (
        p_provinces IS NULL OR p_location_scopes IS NULL OR
        ('country' = ANY(p_location_scopes) AND ('country' = j.location_scope OR ARRAY['country'] && j.listing_location_scopes)) OR
        (j.location_province_code = ANY(p_provinces) AND j.location_scope = ANY(p_location_scopes)) OR
        (p_provinces && j.listing_location_province_codes AND p_location_scopes && j.listing_location_scopes)
      )
      ...
      AND (
        CASE
          WHEN p_interest = 'true' THEN j.is_interested IS TRUE
          WHEN p_interest = 'false' THEN j.is_interested IS FALSE
          WHEN p_interest = 'null' THEN j.is_interested IS NULL
          WHEN p_kind = 'new' AND p_filter_status IS DISTINCT FROM 'entry_level' THEN j.is_interested IS NULL OR j.is_interested IS TRUE
          ELSE true
        END
      )
      ...
      AND (NOT p_has_salary OR p_salary_min IS NULL OR j.salary_min >= p_salary_min)
      AND (NOT p_has_salary OR p_salary_max IS NULL OR j.salary_min <= p_salary_max)
      AND (p_sort_by <> 'salary_min' OR j.salary_min <= 1000000)
    ...
    SELECT
      ...
      row_number() OVER (
        ORDER BY
          ...
          CASE WHEN p_sort_by = 'resume_score' AND p_sort_order = 'asc' THEN resume_score END ASC,
          CASE WHEN p_sort_by = 'resume_score' AND p_sort_order = 'desc' THEN resume_score END DESC,
          CASE WHEN p_sort_by = 'application_date' AND p_sort_order = 'asc' THEN application_date END ASC,
          CASE WHEN p_sort_by = 'application_date' AND p_sort_order = 'desc' THEN application_date END DESC,
          CASE WHEN p_sort_by = 'salary_min' AND p_sort_order = 'asc' THEN salary_min END ASC NULLS LAST,
          CASE WHEN p_sort_by = 'salary_min' AND p_sort_order = 'desc' THEN salary_min END DESC NULLS LAST,
          CASE WHEN p_sort_by = 'repost_count' AND p_sort_order = 'asc' THEN repost_count END ASC,
          CASE WHEN p_sort_by = 'repost_count' AND p_sort_order = 'desc' THEN repost_count END DESC,
          CASE WHEN p_sort_by = 'seen_count' AND p_sort_order = 'asc' THEN seen_count END ASC,
          CASE WHEN p_sort_by = 'seen_count' AND p_sort_order = 'desc' THEN seen_count END DESC,
          job_id ASC
      ) AS row_number
    FROM matched
  )
  , total AS (
    ...
    SELECT ranked.job_id, ranked.row_number
    FROM ranked
    WHERE ranked.row_number > greatest(p_offset, 0)
      AND ranked.row_number <= greatest(p_offset, 0) + least(greatest(p_limit, 1), 1000)
  )
  SELECT paged.job_id::text, total.total_count, paged.row_number
  FROM total LEFT JOIN paged ON true
  ORDER BY paged.row_number;
$$;

REVOKE ALL ON FUNCTION public.job_search_eval(public.jobs, jsonb) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.job_search_eval(public.jobs, jsonb) TO service_role;
